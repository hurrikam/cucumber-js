...
class Master {
  ...
  shouldCauseFailure(status, attemptNumber) {
    ...
    const shouldRetry = status === _status.default.FAILED && attemptNumber <= this.options.retry;

    if (shouldRetry && !isStrict) {
      return false;
    }

    return _lodash.default.includes([_status.default.AMBIGUOUS, _status.default.FAILED, _status.default.UNDEFINED], status) || status === _status.default.PENDING && isStrict;
  }
