...
class TestCaseRunner {
  ...
  async run() {
    ...
    for (let attemptNumber = 1; attemptNumber <= this.maxAttempts; attemptNumber++) {
      const attemptEventData = this.maxAttempts > 1 ? {
      ...
      await this.runSteps();
      const shouldRetry = this.result.status === _status.default.FAILED && attemptNumber < this.maxAttempts;

      if (this.result.status === _status.default.PASSED && attemptNumber > 1) {
        this.result.status = _status.default.FLAKY;
      ...
      this.emit('test-case-finished', { ...attemptEventData,
        result: this.result
      });
